...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim: int, text_upstream_dim: int, downstream_expert: dict, expdir: str, **kwargs):
        ...
        # audio_model
        self.audio_encoder = Conformer(input_dim=80,model_dim=64,num_heads=4,ffn_dim=512,num_layers=12,depthwise_conv_kernel_size=15,)
        # 
        self.speech_embedding = nn.Sequential(nn.Linear(self.modelrc['projector_dim'] // 4, self.modelrc['projector_dim']),
                                             nn.ReLU())
    ...
    # Interface
    def forward(self, mode, padded_feats, feats_length, feat_text, text_mask, labels, records, **kwargs):
        # device = padded_feats[0].device
        # audio_len = torch.IntTensor([len(feat) for feat in features]).to(device=device)
        # T = len(features)
        audio_mask = ~make_pad_mask(feats_length, 0).unsqueeze(1)  # (B, 1, T)
        ...
        features = self.audio_encoder(padded_feats, feats_length)
...
def split_kws_dataset(root_dir):
    ...
    Libriphrase_Origin_path = os.path.join(root_dir.replace('LibriPhrase', 'LibriPhrase_Origin'), 'diff_spk')
    Libriphrase_Noise_path = os.path.join(root_dir.replace('LibriPhrase', 'LibriPhrase_Noise'), 'diff_spk')
    for i in range(1, 5):
        ...
        for line in cur_list:
            anc_pos = [os.path.join(Libriphrase_Origin_path, line[0]), line[2], '1']
            anc_neg = [os.path.join(Libriphrase_Origin_path, line[0]), line[6], line[-2]]
            com_pos = [os.path.join(Libriphrase_Origin_path, line[4]), line[6], '1']
            com_neg = [os.path.join(Libriphrase_Origin_path, line[4]), line[2], line[-2]]
            # wav_path, text, label
        ...
        for line in cur_list:
            anc_pos = [os.path.join(Libriphrase_Noise_path, line[0]), line[2], '1']
            anc_neg = [os.path.join(Libriphrase_Noise_path, line[0]), line[6], line[-2]]
            com_pos = [os.path.join(Libriphrase_Noise_path, line[4]), line[6], '1']
            com_neg = [os.path.join(Libriphrase_Noise_path, line[4]), line[2], line[-2]]
            # wav_path, text, label
            train_cur_list.append((anc_pos[0], anc_pos[1], int(anc_pos[2])))
            train_cur_list.append((anc_neg[0], anc_neg[1], int(anc_neg[2])))
            train_cur_list.append((com_pos[0], com_pos[1], int(com_pos[2])))
